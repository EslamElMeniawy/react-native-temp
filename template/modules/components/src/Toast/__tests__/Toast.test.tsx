...
jest.mock('@modules/theme', () => {
  const useAppTheme = jest.fn(() => baseTheme);
...
const makeTheme = (overrides: Partial<typeof baseTheme> = {}) => ({
  ...baseTheme,
  ...overrides,
  colors: { ...baseTheme.colors, ...(overrides.colors ?? {}) },
});

const setMockTheme = (theme: any) => {
...
const assertDefaultToast = () => {
  ...
  expect(lastColor(screen.getByText('Info Body').props.style)).toBe('#111111');
};
...
describe('ToastManager', () => {
  ...
  it('uses dark theme values when provided', () => {
    setMockTheme(
      makeTheme({
        ...
        colors: {
          ...
          error: '#ff4444',
          errorContainer: '#222222',
          onErrorContainer: '#dddddd',
          tertiary: '#ffaa00',
          tertiaryContainer: '#1a1a1a',
          onTertiaryContainer: '#f5f5f5',
          elevation: { level3: '#101010' },
        },
      }),
    );
    ...
    expect(managerStyle.backgroundColor).toBe('#000000');
